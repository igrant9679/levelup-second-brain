...
async function startServer() {
  ...
  // Trust the platform's reverse proxy (Railway, etc.) so req.protocol/req.secure
  // reflect the real client→edge protocol. This makes the session cookie's
  // `secure` flag get set correctly when the request came in over HTTPS.
  app.set("trust proxy", 1);
  // Configure body parser with larger size limit for file uploads
  // Body limit ~200mb so a 100MB import file (base64 encoded ~133MB) fits.
    ...
    try {
      ...
      if (!emailAlreadySent) {
        ...
        for (const t of expiring7) {
          ...
          const reconnectUrl = siteBase ? `${siteBase}/?goto=accounts&provider=${t.provider}` : "";
          const reconnectBtn = reconnectUrl
